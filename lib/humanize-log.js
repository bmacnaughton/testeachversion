...
const argv = minimist(process.argv.slice(2), {
  ...
  boolean: ['duplicates', 'all', 'template', 'last', 'differences']
})
...
Promise.all(argv._.map(spec => getFiles(spec))).then(results => {
  ...
  // there are errors for a given version then add a .err to the
  // filename (so it won't automatically overwrite good content).
  //
  //
