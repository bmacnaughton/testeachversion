...
Promise.all(argv._.map(spec => getFiles(spec))).then(results => {
  ...
  //   use ubuntu as a base (eventually chosable)
  //   find deltas with other OS results - implies group compare
  //   create output file based on temple
  ...
  groups.forEach(g => {
    ...
    Object.keys(basePackages).forEach(k => {
      ...
      let text = p.ranges.reduce((result, r) => {
        if (argv.all) {
          result.push(`${r.key} ${range(r)} (${r.count})`)
        } else if (r.key === 'pass') {
          result.push(`${range(r)}`)
        }
        return result
