...
//
// Entities are usually package-version pairs but for node builtins
// they are builtin-version pairs. A entity could just be an abstract
// test as well (if builtin is specified as an option).
//
exports.Entity = class Entity {
  ...
  install () {
    ...
    const packages = [this]
    const cd = this.context.dependencies
...
function getTask (task) {
  ...
  if (typeof task === 'string') {
    const parts = task.split(' ').filter(s => s.length)
